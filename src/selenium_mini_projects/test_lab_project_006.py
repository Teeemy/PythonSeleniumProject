...
file_path_fromos = os.getcwd() + r"\opencarttestdata.xlsx"
print(file_path_fromos)


@pytest.mark.parametrize("user_cred", read_credentials_from_excel(file_path_fromos))
@allure.title("Verify adding and searching of username.")
@allure.description("TC#1 - Add and search username in OpenCart website")
def test_opencart_login(user_cred):
    username = user_cred["username"]
    password = user_cred["password"]
    print(username, password)
    opencart_login(username=username, password=password)


def opencart_login(username, password):
